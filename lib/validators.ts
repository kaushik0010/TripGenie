...
export const tripSchema = z.object({
  ...
  duration: z.coerce.number().min(1, { message: "Please enter a duration." }).transform(Number).pipe(z.number().min(1, "Trip must be at least 1 day.")),
  budget: z.coerce.number().min(1, { message: "Please enter a budget." }).transform(Number).pipe(z.number().min(50, "Budget must be at least $50.")),
  currency: z.string(),
